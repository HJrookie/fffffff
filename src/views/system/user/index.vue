      ...
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-row>
          <el-form-item label="用户类型" prop="userType">
            <el-select v-model="form.userType" placeholder="请选择用户类型" filterable clearable style="width: 100%">
...
export default {
  ...
  methods: {
    ...
    handleStatusChange(row) {
      l("row", row, row.status, typeof row.status, row.status === 0);
    ...
    handleResetPwd(row) {
      this.$prompt('请输入"' + row.userName + '"的新密码', "提示", {
        ...
        .then(({ value }) => {
          resetUserPwd(row.userId, md5(value.trim())).then((response) => {
