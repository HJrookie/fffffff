      ...
      <div class="form-container">
        ...
        <el-form ref="form" :model="form" :rules="rules" class="login-form" label-position="right" label-width="120px" size="small">
          <el-form-item prop="certType" label="证件类型">
            <el-select v-model="form.certType" placeholder="请选择证件类型" filterable clearable style="width: 100%">
              <el-option v-for="item in dicts.institutionCertTypes" :key="item.value" :label="item.label" :value="item.value" />
            </el-select>
          </el-form-item>

          <el-form-item prop="idCardOne" label="上传企业证件" class="form-item-card">
            <el-upload :action="uploadUrl" :headers="headers" drag :on-success="idCard1SuccessHandle" :show-file-list="false" accept="image/*">
              <template v-if="idCardOne.imgUrl">
                <template class="">
                  <img class="el-upload-list__item-thumbnail id-card" :src="idCardOne.imgUrl" alt="请上传对应的证件照片" />
              ...
              <template v-else>
                ...
                <div class="el-upload__text">请上传对应的证件照片</div>
              </template>
            </el-upload>
          </el-form-item>

          <el-form-item prop="certNumber" label="证件号">
            <el-input v-model="form.certNumber" placeholder="请输入证件号"></el-input>
          </el-form-item>
          ...
          <el-form-item prop="email" label="邮箱">
            ...
            <el-input :value="form.email" @input="handleEmailChange" placeholder="请输入邮箱" autocomplete="off"></el-input>
          </el-form-item>
...
import { checkRegisterCode, getRegisterCode, registerInstitution } from "@/api/system/user";
    ...
    handleRegister() {
      this.$refs.form.validate((valid) => {
        if (valid) {
          ...
          const { name, certType: cardType, certNumber: idCard, phone: phonenumber, email, password } = formData;

          const data = {
            // phonenumber,
            cardType,
            idCardOne: this.idCardOne.imgId, // 证件文件 id
            idCard, // 证件号
            name,
            email,
