    ...
    >
      ...
      >
        ...
        <template v-else-if="item.inputType === 'inputNumber'">
          <el-input-number v-model="item.value" :min="1000" :max="9999" :label="formatPlaceholder(item)"></el-input-number>
        </template>
...
export default {
  ...
  dicts: ["sys_contest_years", "sys_contest_groups", "sys_contest_rule", "sys_contest_items", "sys_role_others", "sys_apply_status"],
  ...
  created() {
    ...
    this.data.items.forEach((item, i) => {
      ...
      // 初始化选项
      if (item.inputType === "select" && Object.prototype.toString.call(item.options) === "[object Object]") {
          ...
          return new Promise((resolve, reject) => {
            ...
            if (options.ajax) {
              ...
              options
                .ajax(defaultParams)
                .then((res) => {
                  // 把获取到的数据格式化,并且存储下来, 然后下方修改 loading 状态后,组件重新渲染, getOptions 就能获取到最新的数据了
  ...
  methods: {
    ...
    submitForm(callback) {
      const formName = this.data.formConfig.ref || "defaultRef";
      //  l("data", this.$refs[formName], typeof this.$refs[formName]);
